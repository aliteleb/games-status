...
@section('content')
    ...
    <div class="container f-w">
        ...
        {!! Form::open(['route' => isset($page) ? ['admin.pages.update', $page] : 'admin.pages.store', 'method' => isset($page) ? 'PUT' : 'POST']) !!}

        <div class="d-flex flex-wrap">
            <div class="col col-9 p-2">
                <div class=" mb-3">
                    {!! Form::text('title', old('title', $page->title ?? ''), ['class' => 'form-control  form-control-lg', 'placeholder' => __('ui.page_title')]) !!}
                    @error('title')
                ...
                <div class=" mb-3">
                    <div dir="ltr" class="input-group p-0">
                        ...
                        {!! Form::text('slug', old('slug', $page->slug ?? ''), ['dir'=> 'ltr', 'class' => 'form-control   form-control-lg', 'placeholder' => __('ui.page_slug')]) !!}
                    </div>
                    @error('slug')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class=" mb-3">
                    <textarea name="body" id="page_editor">{{ old('body', $page->body ?? '') }}</textarea>
                    @error('body')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col col-3 text-center post-side pt-4">
                ...
                <div class="d-flex flex-column justify-center align-items-center mb-3">
                    ...
                    <img src="{{ isset($page) ? storage('media', '/images/medium/'.$page->featured_image) : (strlen(old('featured_image') > 0) ? storage('media', '/images/medium/'.old('featured_image')) : 'https://dummyimage.com/300x180/8db4ff/000') }}" onclick="change_image(this)" width="260"
                         height="140" class="img-fluid settings-img">
                    {{ Form::hidden('featured_image', isset($page) ? $page->featured_image : '') }}
                ...
                <div class="mb-5">
                    ...
                    {!! Form::text('keywords', old('keywords', $page->keywords ?? ''), ['class' => 'keywords-input', 'placeholder' => __('ui.keywords')]) !!}
                    @error('keywords')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    {!! Form::label('excerpt',  __('ui.page_excerpt')) !!}
                    {!! Form::textarea('excerpt', old('excerpt', $page->excerpt ?? ''), ['class' => 'form-control  form-control-lg', 'placeholder' => __('ui.page_excerpt')]) !!}
                    @error('excerpt')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div>
                    <input type="hidden" name="show_in_footer" class="for-checkbox" value="0">
                    <div class="form-group trigger">
                        <label for="show_in_footer" class="col-form-label">@lang('ui.show_in_footer'):</label>
                        <input type="checkbox" id="show_in_footer" {{ old('show_in_footer', isset($page) ? $page->show_in_footer : false) ? 'checked' : '' }}>
                        <label for="show_in_footer" class="col-form-label trigger-label">@lang('ui.show_in_footer'):</label>
                    </div>
        ...
        <script>
            tinymce.init({
                selector: '#page_editor', // Use a class to target elements for inline editing
